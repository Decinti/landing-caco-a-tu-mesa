...
const variantStyles = {
  primary:
    "border border-accent bg-accent/10 text-foreground hover:bg-accent hover:text-background",
  outline:
    "border border-foreground/30 bg-transparent text-foreground hover:border-accent hover:text-accent",
  ghost:
    "border border-transparent bg-transparent text-foreground hover:text-accent",
} as const;
...
export function WhatsAppButton({
  ...
  return (
    <Link
      ...
      className={`inline-flex items-center justify-center gap-2 font-sans font-medium uppercase tracking-[0.12em] transition-all duration-300 ${variantStyles[variant]} ${sizeStyles[size]} ${fullWidth ? "w-full" : ""} ${className}`}
    >
